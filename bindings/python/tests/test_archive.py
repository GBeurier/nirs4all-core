...
class ArchiveFacadeTests(unittest.TestCase):
    ...
    def test_v3_facade_transports_opaque_refit_bytes(self) -> None:
        observed: list[object] = []
        ...
        with patch.dict(sys.modules, {"nirs4all_core._native": module}):
            reference = write_archive_v3_from_native_payloads(
                "/tmp/refit.n4a",
                {"schema_version": 3},
                {"dagml/portable_refit_package.json": b"package"},
            )
            package = read_portable_refit_package_v3("/tmp/refit.n4a")

        self.assertEqual(reference, {"archive_id": "archive:v3", "archive_sha256": "b" * 64})
        self.assertEqual(package, b'{"schema_version":3}')
        self.assertEqual(
            observed,
            [
                "/tmp/refit.n4a",
