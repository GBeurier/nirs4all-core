...
class ReleaseTopologyManifestTests(unittest.TestCase):
    ...
    def test_optional_upstreams_are_explicit_and_match_python_extras(self) -> None:
        ...
        self.assertEqual(extras["dag-ml"], ["dag-ml>=0.3.23,<0.4"])
        self.assertIn("nirs4all-methods>=1.0.13,<2", extras["methods"])
        self.assertIn("pls4all>=1.0.13,<2", extras["methods"])
        self.assertIn("scikit-learn>=1.3", extras["methods"])
        self.assertIn("nirs4all-formats>=0.2.9", all_extra)
        self.assertIn("nirs4all-io>=0.1.14", all_extra)
        self.assertIn("dag-ml>=0.3.23,<0.4", all_extra)
        self.assertIn("nirs4all-methods>=1.0.13,<2", all_extra)
        self.assertIn("pls4all>=1.0.13,<2", all_extra)
