...
The initial numeric gates compare the JavaScript/WASM, Python, Rust, and R
bindings against full Python `nirs4all` on four shared JSON/YAML pipeline
fixtures:
...
`sklearn.cross_decomposition.PLSRegression`. The WASM, Python, Rust, and R
tests then execute the same fixture descriptors through their
`runPortablePipeline()`, `run_portable_pipeline()`,
`run_portable_pipeline_with_library()`, and
`nirs4all_run_portable_pipeline()` APIs and compare split indices, targets,
RMSE, predictions, and selected sweep component within the declared tolerances.
The WASM gate also reuses the serialized selected model through
`predictPortablePipeline()` and checks that predictions on the held-out rows
match the selected run output.
...
The remaining binding work is to promote the same execution comparison to the
MATLAB/Octave aggregate binding once its upstream method surface can execute the
portable KS/SNV/Savitzky-Golay/PLS subset. Until then that binding remains a
parser/domain surface and must not claim execution parity.
